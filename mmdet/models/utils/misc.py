...
def _filter_gt_instances_by_score_domain(batch_data_samples: SampleList,
                                         weight_list: list,
                                         score_thr: float) -> SampleList:
    """Filter ground truth (GT) instances by score.
    ...
    Returns:
        SampleList: The Data Samples filtered by score.
    """
    for data_samples, weight in zip(batch_data_samples, weight_list):
        assert 'scores' in data_samples.gt_instances, \
            'there does not exit scores in instances'
        if data_samples.gt_instances.bboxes.shape[0] > 0:
            data_samples.gt_instances = data_samples.gt_instances[
                data_samples.gt_instances.scores > np.sqrt(weight)*score_thr]
    return batch_data_samples
...
def filter_gt_instances_domain(batch_data_samples: SampleList,
    ...
    batch_data_samples = _filter_gt_instances_by_score_domain(
        batch_data_samples, weight_list, score_thr)
