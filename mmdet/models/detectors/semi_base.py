...
class SemiBaseDetector(BaseDetector):
    ...
    def loss_by_pseudo_instances_domain(self,
        ...
        Returns:
            dict: A dictionary of loss components
        """
        batch_data_samples = filter_gt_instances_domain(
            batch_data_samples, weight_list=weight_list, score_thr=self.semi_train_cfg.cls_pseudo_thr)
        losses = self.student.loss(batch_inputs, batch_data_samples)
        pseudo_instances_num = sum([
            len(data_samples.gt_instances)
            for data_samples in batch_data_samples
        ...
        unsup_weight = self.semi_train_cfg.get(
            'unsup_weight', 1.) if pseudo_instances_num > 0 else 0.
        return rename_loss_dict('unsup_domain_adaptive_',
                                reweight_loss_dict(losses, unsup_weight))

    def loss_by_gt_instances_aug(self, batch_inputs: Tensor,
                                 batch_data_samples: SampleList) -> dict:
        """Calculate losses from a batch of inputs and ground-truth data
        samples.
        ...
        losses = self.student.loss(batch_inputs, batch_data_samples)
        return rename_loss_dict('aug_', reweight_loss_dict(losses, 1.0))
