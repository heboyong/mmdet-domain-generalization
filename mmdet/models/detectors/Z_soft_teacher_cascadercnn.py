...
class SoftTeacherCascade(SemiBaseDetector):
    ...
    def rcnn_cls_loss_by_pseudo_instances(self, x: Tuple[Tensor],
        ...
        for stage in range(self.student.roi_head.num_stages):
            self.student.roi_head.current_stage = stage
            stage_loss_weight = self.student.roi_head.stage_loss_weights[stage]
            if self.student.roi_head.with_bbox or self.student.roi_head.with_mask:
                bbox_assigner = self.student.roi_head.bbox_assigner[stage]
                bbox_sampler = self.student.roi_head.bbox_sampler[stage]
                # assign gts and sample proposals
                sampling_results = []
                for i in range(num_imgs):
                    # rename rpn_results.bboxes to rpn_results.priors
                    rpn_results = rpn_results_list[i]
                    # rpn_results =copy.deepcopy(rpn_results_list[i])
                ...
                bbox_head = self.student.roi_head.bbox_head[stage]

                selected_bboxes = [res.priors for res in sampling_results]
                rois = bbox2roi(selected_bboxes)
                bbox_results = self.student.roi_head._bbox_forward(stage, x, rois)
                bbox_results.update(rois=rois)
                # cls_reg_targets is a tuple of labels, label_weights,
                # and bbox_targets, bbox_weights
                cls_reg_targets = bbox_head.get_targets(
                    sampling_results, self.student.train_cfg.rcnn[stage])
                ...
                with torch.no_grad():
                    ...
                    neg_inds = cls_reg_targets[
                                   0] == bbox_head.num_classes
                    # cls_reg_targets[1] is label_weights
                    cls_reg_targets[1][neg_inds] = bg_score[neg_inds].detach()

                losses1 = bbox_head.loss(
                    bbox_results['cls_score'], bbox_results['bbox_pred'], rois,
                    *cls_reg_targets)
                # cls_reg_targets[1] is label_weights

                losses[f's{stage}.loss_cls'] = losses1['loss_cls'] * len(
                    cls_reg_targets[1]) / max(sum(cls_reg_targets[1]), 1.0) * stage_loss_weight
        return losses
    ...
    def rcnn_reg_loss_by_pseudo_instances(
        ...
        losses = dict()
        for stage in range(self.student.roi_head.num_stages):
            losses.update({f's{stage}.loss_bbox': roi_losses[f's{stage}.loss_bbox']})
        return losses
